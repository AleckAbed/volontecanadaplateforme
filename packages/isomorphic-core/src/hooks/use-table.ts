...
export function useTable<T extends AnyObject>(
  ...
  function applyFilters() {
    ...
    const hasActiveFilters = Object.entries(filters).some(([columnId, filterValue]) => {
      if (Array.isArray(filterValue)) {
        // Pour les dates, vérifier si au moins une date est définie
        if (typeof filterValue[1] === 'object') {
          return filterValue[0] !== null || filterValue[1] !== null;
        }
        // Pour les plages numériques, vérifier si les valeurs ne sont pas vides
        return filterValue[0] !== '' || filterValue[1] !== '';
      }
      // Pour les strings, vérifier si la valeur n'est pas vide
    ...
    // Si aucun filtre n'est actif et pas de recherche, retourner toutes les données triées
    ...
    return (
      sortedData
        .filter((item) => {
          // Si aucun filtre n'est actif, passer tous les éléments
          if (!hasActiveFilters) {
            return true;
          }

          // Vérifier que l'élément correspond à tous les filtres actifs
          const matchesAllFilters = Object.entries(filters).every(
            ([columnId, filterValue]) => {
              ...
              if (Array.isArray(filterValue)) {
                if (typeof filterValue[1] === 'object') {
                  ...
                  if (filterValue[0] === null && filterValue[1] === null) {
                    return true; // Pas de filtre de date, on accepte
                  }
                ...
                if (filterValue[0] === '' && filterValue[1] === '') {
                  return true; // Pas de filtre numérique, on accepte
                }
              ...
              if (isString(filterValue) && !Array.isArray(filterValue)) {
                // Filtre string
                if (filterValue === '') {
                  return true; // Pas de filtre string, on accepte
                }
        ...
        .filter((item) => {
          if (!searchTerm) {
            return true; // Pas de recherche, on accepte tous les éléments
          }
